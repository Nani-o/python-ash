...
"""Domain model classes for AAP objects."""
...
class JobTemplate(BaseObject):
    def __init__(self, api, data):
        ...
        self.absolute_url = requests.compat.urljoin(
            self.api.base_url,
            f"execution/templates/job-template/{self.id}"
        )
    ...
    def jobs(self):
        return self.api.retrieves_objects(
            "jobs", baseuri=f"{self.uri}/jobs/",
...
class Inventory(BaseObject):
    def __init__(self, api, data):
        ...
        self.absolute_url = requests.compat.urljoin(
            self.api.base_url,
            f"execution/infrastructure/inventories/inventory/{self.id}/details"
        )
...
class Host(BaseObject):
    def __init__(self, api, data):
        ...
        self.absolute_url = requests.compat.urljoin(
            self.api.base_url,
            f"execution/infrastructure/inventories/inventory/{self.inventory}/hosts/{self.id}/details"
        )
...
class Project(BaseObject):
    def __init__(self, api, data):
        ...
        self.absolute_url = requests.compat.urljoin(
            self.api.base_url,
            f"execution/projects/{self.id}/details"
        )
...
class Job(BaseObject):
    def __init__(self, api, data):
        ...
        self.absolute_url = requests.compat.urljoin(
            self.api.base_url,
            f"execution/jobs/playbook/{self.id}/output"
        )
